...
var RoomController = (function () {
    function RoomController($scope) {
        ...
        this.hub.on('UpdateTotaling', function (totaling) {
            $scope.$apply(function () {
                totaling.forEach(function (a) {
                    var option = $scope.options.filter(function (opt) { return opt.text == a.label; })[0];
            ...
            if (chartInstance == null) {
                ...
                data.forEach(function (a, n) { return chartInstance.segments[n].value = a.value; });
        ...
        $.connection.hub.start().then(function () { return _this.hub.invoke('EnterRoom', window._app.roomNumber); }).then(function (data) {
            $scope.$apply(function () {
                $scope.options = data;
            });
        });
    ...
    RoomController.prototype.postAnswer = function (option) {
        if (option.selected) {
            ...
            this.$scope.options.forEach(function (opt) { return opt.selected = false; });
